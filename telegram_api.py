...
def call(method: str, payload: dict):
    ...
    if response.status_code == 429 or response.status_code >= 500:
        raise TelegramError(
            f"Transient Telegram error: {response.status_code} {body}"
        )
    if response.status_code >= 400 or not body.get("ok"):
        raise TelegramError(
            f"Telegram rejected {method}: {response.status_code} {body}"
        )
    return body["result"]
...
def send_quiz(question, chat_id=None):
    ...
    if len(correct_ids) != 1:
        raise TelegramError("Exactly one correct option is required")
    ...
    return call(
        "sendPoll",
        {
            ...
            "is_anonymous": True,
            # The application already balances positions programmatically.
