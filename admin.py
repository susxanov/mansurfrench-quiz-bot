...
/force morning — полный тест утреннего цикла, включая выходные
/force evening — полный тест вечернего цикла, включая выходные
...
def _manual_prepare(session: str, force: bool = False):
    ...
    try:
        result = send_for_approval(today, session, force=force)
...
def handle_message(text: str, chat_id: int, user_id: int):
    ...
    if cmd in {"/start", "/help"}:
        ...
        threading.Thread(target=_manual_prepare, args=(arg, False), daemon=True).start()
        send_text("Подготовка запущена.", chat_id)
    elif cmd == "/force" and arg in {"morning", "evening"}:
        threading.Thread(target=_manual_prepare, args=(arg, True), daemon=True).start()
        send_text("Принудительная проверка полного цикла запущена.", chat_id)
    elif cmd == "/pending":
        with session_scope() as db:
