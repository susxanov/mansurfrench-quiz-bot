...
def send_for_approval(target_date: date, session: str, force: bool = False) -> str:
    block = prepare_block(target_date, session, force=force)
    ...
    send_text(
        ...
        reply_markup={
            "inline_keyboard": [[
                ...
                {"text": "🔄 Пересоздать", "callback_data": f"{'regenerate_force' if force else 'regenerate'}:{block.id}"},
            ]]
        },
    )
...
def regenerate_block_by_id(block_id: int, force: bool = False) -> str:
    with session_scope() as db:
        block = db.get(DailyBlock, block_id)
        if not block:
            raise RuntimeError("Блок не найден.")
        target_date, session = block.target_date, block.session
    prepare_block(target_date, session, replace=True, force=force)
    return send_for_approval(target_date, session, force=force)
